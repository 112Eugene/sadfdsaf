...
def setup_map_admin(app: FastAPI):
    """
    Устанавливает новую админ-панель на основе карты Leaflet
    Доступна по адресу: /admin-map/
    
    Требует следующие существующие API эндпоинты:
    - /api/admin/radar - для отображения игроков на карте
    - /api/admin/heatmap/{quest_id} - для тепловой карты
    - /api/admin/dicts - для получения списков квестов и шагов
    """
    
    @app.get("/admin-map/")
    async def get_admin_map():
        """
        Возвращает HTML админ-панели с картой Leaflet
        
        Интерфейс включает:
        - Карта OpenStreetMap в центре
        - Боковое меню слева (320px, фиксированное)
        - Верхняя панель статистики
        - Модальные окна для создания квестов и городов
        - Система уведомлений
        """

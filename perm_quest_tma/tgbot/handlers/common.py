...
async def start_cmd(message: Message, state: FSMContext):
    ...
    text = (
        ...
        f"Баланс кошелька: *{user_data.coins} монет*\n\n"
...
async def user_profile_cmd(event: Message | CallbackQuery, state: FSMContext):
    ...
    if user_data.income_buffer > 0:
        builder.button(text=f"💰 Собрать прибыль (+{user_data.income_buffer} монет)", callback_data="claim_passive_income")
    ...
    builder.button(text=f"🧪 Купить эликсир Кармы ({cfg.karma_elixir_price} монет)", callback_data="buy_karma_elixir")

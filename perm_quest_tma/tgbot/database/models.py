...
class StepBranchesSchema(BaseModel):
    # Теперь branches поддерживает расширенную логику (вероятности, условия)
...
class User(Base):
    ...
    # Флаги игрока (для мета-геймплея и условий в графах)
...
class ShopItem(Base):
    ...
    generates_income: Mapped[bool] = mapped_column(Boolean, default=False)
    income_per_hour: Mapped[int] = mapped_column(Integer, default=0)
    market_id: Mapped[Optional[int]] = mapped_column(ForeignKey("quest_markets.id", ondelete="SET NULL"), nullable=True)
